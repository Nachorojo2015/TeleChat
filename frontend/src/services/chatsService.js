...
export const deleteChat = async (chatId) => {
  try {
    let response = await fetch(`http://localhost:3000/chats/delete/${chatId}`, {
      method: "DELETE",
      credentials: "include", // si usas cookies
    });
    ...
    if (response.status === 401) {
      ...
      // Reintentar la petición original con el nuevo token
      response = await fetch(`http://localhost:3000/chats/delete/${chatId}`, {
        method: "DELETE",
        credentials: "include",
      });
    }
    ...
    if (!response.ok) {
      throw new Error("Error al borrar el chat");
    }
